...
android {
  namespace = "dev.sparkison.tv.libmpv"
  compileSdk = 36
  // No explicit pin here -- android/local.properties' ndk.dir (project-wide,
  // machine-local) already fixes which NDK every module builds against, same
  // as :app's own flutter.ndkVersion resolves to. Declaring a different
  // literal version here (Plezy pins NDK 29 for its 16 KB page-size-capable
  // libc++) makes AGP reject the mismatch against ndk.dir instead of just
  // using it: [CXX1104] "NDK from ndk.dir ... disagrees with
  // android.ndkVersion". The libc++_shared.so this NDK produces for the
  // glue lib isn't what ships anyway: the app packages the mpv-build
  // tarball's own newer copy with top merge priority (see
