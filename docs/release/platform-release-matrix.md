...
| Linux desktop | Flutter Linux desktop support, GTK 3 development files (`gtk+-3.0` pkg-config metadata), `clang++` or configured `CXX`, CMake/Ninja, and `libmpv.so.2` (or `.1`/`.so`) on the host. | Optional package signing placeholder for deb/rpm/AppImage release keys; no private key belongs in git. | Build and orchestrator wiring now pass on the dev executor (NVIDIA RTX 4070 SUPER, driver 580.159.03, Flutter 3.44.2, media_kit_video 2.0.1). Outstanding: portable bundle of `libmpv.so.*` + transitive FFmpeg/libplacebo for AppImage/snap/flatpak; license notices for the bundled libraries; release signing keys. `integration_test/desktop_playback_smoke_test.dart` has two pre-existing assertion failures (stale test expectations; runtime behavior is correct) — see `docs/migration/desktop-libmpv-feasibility.md`. | Unblocked for development; blocked for portable distribution until `libmpv.so.*` is bundled and license notices are in place. |
