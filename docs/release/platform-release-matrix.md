...
| macOS desktop | NON-BLOCKING/GATED. macOS runner with Xcode command-line tools, Flutter macOS desktop support, codesign/notarization tooling, and the media_kit (AVFoundation-backed) playback path — no libmpv/MPVKit dependency. | Apple Developer ID certificate, notarization credentials, and optional App Store distribution certificate placeholders must be supplied outside git. | macOS signing and native playback packaging are not proven here. A native in-process libmpv backend was prototyped and reverted; libmpv/MPVKit is not planned for macOS, so those licensing gates do not apply to this platform. | NON-BLOCKING/GATED for this Desktop+Android release. It must not block Linux, Windows, Android, or Android TV release criteria. |
...
- MPVKit/libmpv is not planned for iOS/tvOS: GPL-3.0 is incompatible with App Store distribution for this app, a firm product decision rather than a review gate awaiting approval. Not applicable to macOS either — libmpv is not planned there; macOS stays on media_kit.
