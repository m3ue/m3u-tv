...
| macOS desktop | NON-BLOCKING/GATED. macOS runner with Xcode command-line tools, Flutter macOS desktop support, codesign/notarization tooling, and the custom in-process libmpv playback path (`DesktopLibmpvBackend`), bundling MPVKit (`Mpv.framework`) via Swift Package Manager, pinned to `mpvkit/MPVKit@1.0.0` — CocoaPods was tried first but that pod has no macOS binary slices. Requires macOS 12.0+ (bumped from 10.15). | Apple Developer ID certificate and notarization credentials must be supplied outside git; no App Store distribution certificate is used since this app is not published through the Mac App Store. | macOS signing and native playback packaging use the same libmpv approach as Linux/Windows, verified via a bundle-presence/`otool` check in the `build-macos` release job. MPVKit is GPL-3.0; this is accepted for direct-to-GitHub DMG distribution (not App Store) per the GPL Policy Gate in `docs/release/license-notices-checklist.md`. | NON-BLOCKING/GATED for this Desktop+Android release. It must not block Linux, Windows, Android, or Android TV release criteria. |
| iOS/iPadOS | NON-BLOCKING/GATED. macOS runner with Xcode, Flutter iOS support, provisioning profiles, device/simulator smoke targets, and AVKit/AVPlayer as the permanent playback path. | Apple Distribution certificate and provisioning profile placeholders must be supplied by CI secrets or Xcode-managed signing. | iOS release signing and App Review readiness are not proven here. MPVKit is not planned — GPL-3.0 is incompatible with App Store distribution for this app, a firm decision rather than a gate awaiting review. | NON-BLOCKING/GATED for this Desktop+Android release. It must not block Linux, Windows, Android, or Android TV release criteria. |
| tvOS | NON-BLOCKING/GATED and BLOCKED. Custom Flutter tvOS embedder, Xcode tvOS runner, Siri Remote/gamepad input bridge, AVKit playback path, plugin implementation audit, and device/simulator QA are required. | Apple tvOS distribution signing placeholders must be supplied outside git. | The pinned Flutter toolchain does not expose an official `flutter build tvos` command. tvOS also needs community/custom embedder proof and missing tvOS plugin implementations resolved. MPVKit is not planned here either, independent of the embedder blocker, for the same GPL/App Store reason as iOS. | NON-BLOCKING/GATED and BLOCKED. Do not claim production readiness until the custom embedder, plugin audit, store/legal review, and remote/playback QA pass. |
...
- macOS production packaging must verify the bundled MPVKit framework and dynamic dependencies (see `build-macos` job's "Verify macOS bundle and libmpv linkage" step) and include license notices before public release; real-device playback acceptance evidence (duration detection, scrubbing, CPU usage) remains open.
...
- macOS and iOS/iPadOS require Xcode host evidence, provisioning/codesigning, and notarization (macOS) or App Store review preparation (iOS/iPadOS) before any Apple release claim.
- MPVKit/libmpv is not planned for iOS/tvOS: GPL-3.0 is incompatible with App Store distribution for this app, a firm product decision rather than a review gate awaiting approval. macOS is different: it is not distributed through the Mac App Store, so it uses the same libmpv backend (bundling MPVKit) as Linux/Windows.
