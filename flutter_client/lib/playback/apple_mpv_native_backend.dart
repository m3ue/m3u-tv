...
// filters it down to its own [_viewId] in [_applyEvent]). Do not wrap in
// .asBroadcastStream() -- its pause/resume semantics break resubscription
// across sequential loads/disposes.
Stream<AppleMpvEvent>? _sharedMpvEvents;
Stream<AppleMpvEvent> _mpvEvents(EventChannel channel) {
  return _sharedMpvEvents ??= channel.receiveBroadcastStream().map((raw) {
    final map = Map<String, Object?>.from(raw! as Map<Object?, Object?>);
    return AppleMpvEvent.fromMap(map);
  });
}
...
/// `_viewId` the Dart side generates up front so the platform view can be
/// created and attached before `load()` is ever called, and subtitles are
/// rendered natively (mpv's own libass compositing), so this adapter does
/// not implement `SubtitleControllerProvider`.
class AppleMpvNativeBackend implements PlayerAdapter, PlatformViewProvider {
  ...
  final List<AppleMpvEvent> _pendingEvents = <AppleMpvEvent>[];
  ...
  Future<void> load(PlaybackSource source) async {
    ...
    try {
      ...
      if (!ok) {
        ...
        final code = response?['code'] as String? ?? 'apple-mpv-load-failed';
        final error = code == AppleMpvUnavailableException.unavailableCode
            ? AppleMpvUnavailableException(message)
            : PlaybackException(
                message: message,
      ...
      final pendingEvents = List<AppleMpvEvent>.of(_pendingEvents);
      _pendingEvents.clear();
  ...
  void _handleEvent(AppleMpvEvent event) {
    if (_disposed) return;
    if (event.kind == AppleMpvEventKind.unknown) return;
  ...
  void _applyEvent(AppleMpvEvent event) {
    if (event.viewId != _viewId) return;
    ...
    if (event.kind == AppleMpvEventKind.error) {
      _errorEmitted = true;
      final error = PlaybackException(
        message: event.message ?? 'apple mpv error',
        backend: capabilities.backend,
        code: event.code ?? 'apple-mpv-error',
    ...
    if (event.kind == AppleMpvEventKind.endFile &&
        _readyCompleter != null &&
        !_readyCompleter!.isCompleted) {
      _errorEmitted = true;
      final error = PlaybackException(
        message: 'apple mpv ended before FILE_LOADED',
        backend: capabilities.backend,
        code: 'apple-mpv-ended-before-ready',
        recoverable: true,
      );
      _failReady(error);
      return;
    ...
    if (effectiveSource == null && event.kind != AppleMpvEventKind.shutdown) {
      return;
    }

    final nextState = AppleMpvEventReducer.reduce(
      _state,
      event,
    ...
    if (_readyCompleter != null &&
        !_readyCompleter!.isCompleted &&
        event.kind == AppleMpvEventKind.fileLoaded) {
  ...
  void _ensureNotDisposed() {
    if (_disposed) {
      throw StateError('AppleMpvNativeBackend is disposed');
    }
  }
}

class AppleMpvUnavailableException extends PlaybackException {
  AppleMpvUnavailableException(String message)
    : super(
        message: message,
        backend: PlaybackBackend.appleMpvNative,
...
class AppleMpvEvent {
  ...
  factory AppleMpvEvent.fromMap(Map<String, Object?> map) {
    return AppleMpvEvent(
  ...
  static AppleMpvEventKind _kindFromString(String? value) {
    return switch (value) {
      'START_FILE' => AppleMpvEventKind.startFile,
      'FILE_LOADED' => AppleMpvEventKind.fileLoaded,
      'PLAYBACK_RESTART' => AppleMpvEventKind.playbackRestart,
      'VIDEO_RECONFIG' => AppleMpvEventKind.videoReconfig,
      'END_FILE' => AppleMpvEventKind.endFile,
      'STOP' => AppleMpvEventKind.stop,
      'QUIT' => AppleMpvEventKind.quit,
      'ERROR' => AppleMpvEventKind.error,
      'SHUTDOWN' => AppleMpvEventKind.shutdown,
      _ => AppleMpvEventKind.unknown,
    };
  }
...
class AppleMpvEventReducer {
  const AppleMpvEventReducer._();
  ...
  static PlaybackState reduce(
    ...
    const backend = PlaybackBackend.appleMpvNative;

    switch (event.kind) {
      case AppleMpvEventKind.startFile:
      ...
      case AppleMpvEventKind.fileLoaded:
        return current.copyWith(
          backend: backend,
      ...
      case AppleMpvEventKind.playbackRestart:
      ...
      case AppleMpvEventKind.videoReconfig:
        return current.copyWith(
          backend: backend,
          videoAspectRatio: event.videoAspectRatio ?? current.videoAspectRatio,
        );
      case AppleMpvEventKind.endFile:
        return current.copyWith(
          backend: backend,
          status: PlaybackStatus.completed,
        );
      case AppleMpvEventKind.stop:
      case AppleMpvEventKind.quit:
      case AppleMpvEventKind.shutdown:
        return current.copyWith(
          backend: backend,
          status: PlaybackStatus.stopped,
        );
      case AppleMpvEventKind.error:
        // Errors are emitted as PlaybackError; state is left unchanged.
        return current;
      case AppleMpvEventKind.unknown:
        return current;
    }
  }
}
