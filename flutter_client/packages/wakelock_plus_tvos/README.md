...
The tvOS implementation of [`wakelock_plus`](https://pub.dev/packages/wakelock_plus).

> Generated by [`flutter-tvos plugin port`](https://github.com/fluttertv/flutter-tvos)
> from `wakelock_plus`. Read `PORTING_REPORT.md` before publishing.

## Usage

This is a federated plugin implementation. Apps that already depend on
`wakelock_plus` and target tvOS only need to add this package as a
dependency:

```yaml
dependencies:
  wakelock_plus: ^<latest>
