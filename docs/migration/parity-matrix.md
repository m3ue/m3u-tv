...
| tvOS | Playback working. Custom Xcode-managed runner builds and runs; native mpv playback confirmed (video, audio, subtitles correct). App Store submission proof (signing, TestFlight, remote-input review) still pending. | Native mpv (`edde746/MPVKit`, `appleMpvNative`) primary, `AppleAvKitBackend` fallback. One known open cosmetic bug: playback overlay renders "boxed in" relative to the video. | Does not block Android release track. |
| iOS | Playback working, confirmed via click-testing on iOS Simulator (video, audio, no crash on back navigation). App Store readiness not claimed. | Native mpv (`edde746/MPVKit`, `appleMpvNative`) primary, `AppleAvKitBackend` fallback. `MediaKitIosAdapter` has been removed. | Does not block Android release track. |
| macOS | Playback working, confirmed via click-testing (video, audio, subtitles, track switching, seeking, clean teardown). Signing and notarization not proven. | Native mpv (`edde746/MPVKit`, `macMpvNative`) primary; no automatic fallback for main playback currently. media_kit remains only for Multiview, which has a known regression (see below). | Does not block Android release track. |
...
| media_kit Apple | Removed from the main-playback fallback chain on iOS and macOS. Still used for the macOS Multiview surface only, where it is a known regression: `buildMultiviewTilePlayer()` still hard-codes `MediaKitDesktopAdapter`, but `media_kit_libs_macos_video` was removed from `pubspec.yaml`, so it likely has no vendored native library to render with on macOS anymore. Not tested or fixed yet. | macOS (Multiview only). |
| libmpv desktop | Active for Linux/Windows only; gated on release-packaging work (signing, bundled runtime licensing). Not used on macOS, which now uses native mpv via MPVKit instead. | Linux, Windows. |
